...
function tree($id, $dep)
{
  ...
  $db->query("set names utf8");
    ...
    $rs              = $conn->fetchAll();
    $rows            = count($rs);
    $str             = '<div style="position:relative;left:40px;border:1px solid #333; padding:5px 10px; margin:5px auto; background:#666; color:#fff;">';
    for ($i          = 0; $i < $rows; $i++) {
      $str          .= "<a href='#'>";
    ...
    //     $str     .= $fidcount.'</a>';
    //     $str     .= ':';
    //     $sql2     = "select * from msg_board where id = $fidcount";设置
    //   $str       .= $rs[$i]['id'].'</a>';
    //   $fidcount   = $rs[$i]['fid'];
    //   for ($j     = 0; $j < ($rs[$i]['dep']); $j++) {
    //     $str     .= '回复';
    ...
    //     $sql2     = "select * from msg_board where id = $fidcount";
      ...
      echo '</div>';
      $str           = '<div style="border:1px solid #333; padding:5px 10px; margin:5px auto; background:#111; color:#fff;">';
